...
description
Translate academic PDFs into Chinese with terminology control, OCR detection, text extraction, figure preservation, quality checking, and figure-aware PDF output.
Academic PDF Translation Skill
...
In final PDF mode, the exporter also attempts to create a second PDF named
from the article's Chinese title, for example:
text
￼
￼
output/通过三功能二元DES分离非缩合木质素.pdf
The fixed PDF path above is always kept. The title-named PDF is generated from
output/translation_final.md by first reading the Chinese title under
题目与作者, then falling back to the article-title translation result. If no
usable Chinese title can be found, only the fixed PDF is kept and the report
explains why.
...
12. When the title-named PDF is generated, sanitize Windows-illegal filename characters, limit the title filename to 60 characters, and add _1, _2, and so on if the file already exists.
13. The PDF report must list the fixed PDF path, the Chinese-title PDF path, the Chinese filename used, and whether the filename was truncated.
Output Requirements
Default outputs:
...
9. output/<中文文章题目>.pdf when a Chinese title can be identified safely
